...
import {
  importInfostealerData,
  validateInfostealerData,
  getJsonSchema,
  InfostealerBatchImportSchema,
} from './adapters/infostealerDataAdapter.js';
...
const ENABLE_INFOSTEALER_API = (process.env.ENABLE_INFOSTEALER ?? '1') !== '0';
...
const VALID_PROFILES = ['full', 'wordpress', 'infostealer', 'email', 'quick', 'github'] as const;
type ScanProfileType = typeof VALID_PROFILES[number];

// Profile documentation for API discovery
const PROFILE_DOCS: Record<ScanProfileType, { description: string; modules: string[]; tier: string; estimatedDuration: string }> = {
  full: {
    description: 'Complete security scan with all modules enabled',
    modules: ['shodan', 'tls', 'spf_dmarc', 'tech_stack', 'nuclei', 'endpoint_discovery', 'config_exposure', 'document_exposure', 'web_archive', 'dns_twist', 'db_port_scan'],
    tier: 'tier1 + tier2',
    estimatedDuration: '10-30 minutes'
  },
  wordpress: {
    description: 'WordPress-focused scan for plugin vulnerabilities and misconfigurations',
    modules: ['wp_plugin_quick_scan', 'wp_vuln_resolver', 'tech_stack', 'tls', 'config_exposure'],
    tier: 'tier1',
    estimatedDuration: '5-10 minutes'
  },
  infostealer: {
    description: 'Credential exposure and breach detection scan',
    modules: ['infostealer_probe', 'breach_directory', 'github_secret_search', 'email_bruteforce_surface'],
    tier: 'tier1',
    estimatedDuration: '5-15 minutes'
  },
  email: {
    description: 'Email security configuration scan (SPF, DMARC, MX)',
    modules: ['spf_dmarc', 'email_bruteforce_surface', 'dns_zone_transfer'],
    tier: 'tier1',
    estimatedDuration: '2-5 minutes'
  },
  quick: {
    description: 'Fast reconnaissance scan for immediate insights',
    modules: ['tech_stack', 'tls', 'shodan', 'spf_dmarc'],
    tier: 'tier1',
    estimatedDuration: '2-5 minutes'
  },
  github: {
    description: 'GitHub-focused scan for exposed secrets and repository issues',
    modules: ['github_secret_search', 'github_domain_extractor', 'trufflehog', 'github_confidence_classifier'],
    tier: 'tier1',
    estimatedDuration: '5-15 minutes'
  }
};
...
// === Custom Infostealer Data Import Endpoints ===

/**
 * GET /infostealer/schema
 * Returns the JSON schema for custom infostealer data import
 */
if (ENABLE_INFOSTEALER_API) app.get('/infostealer/schema', (_req, res) => {
  res.json({
    schema: getJsonSchema(),
    description: 'Schema for importing custom infostealer/breach data. Use POST /infostealer/import to submit data.',
    example: {
      batch_id: 'batch-001',
      source_system: 'your-intel-platform',
...
/**
 * POST /infostealer/validate
 * Validates custom infostealer data without importing
 */
if (ENABLE_INFOSTEALER_API) app.post('/infostealer/validate', async (req, res) => {
  try {
    const validation = validateInfostealerData(req.body);

    if (validation.valid) {
      return res.json({
        valid: true,
        record_count: validation.recordCount,
        message: 'Data is valid and ready for import',
      });
    } else {
    ...
    log.error({ err: error }, 'Infostealer validation error');
    return res.status(500).json(createError(ErrorCode.INTERNAL_ERROR, 'Validation failed', {
      message: safeErrorMessage(error, 'Validation error'),
    }));
  }
});

/**
 * POST /infostealer/import
 * Import custom infostealer/breach data for a scan
 *
 * Body: InfostealerBatchImport schema
 * Query params:
 *   - scan_id: Required. The scan to attach findings to.
 *   - create_scan: Optional. If 'true' and no scan_id provided, creates a new scan.
 *   - domain: Required if create_scan is true. Domain for the new scan.
 */
if (ENABLE_INFOSTEALER_API) app.post('/infostealer/import', async (req, res) => {
  try {
    const { scan_id, create_scan, domain } = req.query;
    ...
    if (!validation.valid) {
      return res.status(400).json({
        success: false,
        errors: validation.errors,
        message: 'Invalid data format - use POST /infostealer/validate to check your data',
      });
    }

    let scanId = scan_id as string;

    // Create a new scan if requested
    if (create_scan === 'true' && !scanId) {
      const domainStr = (domain as string) || req.body.target_domain;
      if (!domainStr || !isValidDomain(domainStr)) {
        return res.status(400).json(Errors.invalidDomain(domainStr || '(not provided)'));
      }

      const normalizedDomain = normalizeDomain(domainStr);
      scanId = nanoid(12);

      // Insert scan record using database service
      await database.insertScan({
        ...
        findings_count: 0,
        artifacts_count: 0,
        metadata: { profile: 'infostealer', company_name: normalizedDomain },
      });

      log.info({ scanId, domain: normalizedDomain }, 'Created new scan for infostealer import');
    }

    if (!scanId) {
      return res.status(400).json({
        success: false,
        error: 'scan_id is required. Provide scan_id query param or set create_scan=true with domain.',
      });
    }

    // Verify scan exists using database service
    ...
    const parsedData = InfostealerBatchImportSchema.parse(req.body);

    // Set target_domain from scan if not specified
    if (!parsedData.target_domain) {
      parsedData.target_domain = scan.domain;
    }

    const result = await importInfostealerData(parsedData, scanId);

    log.info({
      scanId,
      batchId: result.batch_id,
      recordsProcessed: result.records_processed,
      findingsCreated: result.findings_created,
    }, 'Infostealer data imported');
    ...
    log.error({ err: error }, 'Infostealer import error');
    return res.status(500).json(createError(ErrorCode.INTERNAL_ERROR, 'Import failed', {
      message: safeErrorMessage(error, 'Import error'),
    }));
  }
});
...
// Startup validation
import { runStartupValidation, getDisabledModules } from './core/toolValidation.js';

const startupValidation = await runStartupValidation();
const disabledModules = getDisabledModules(startupValidation.tools);

if (disabledModules.length > 0) {
  log.warn({ disabledModules }, 'Some modules are disabled due to missing external tools');
}
...
app.listen(port, '127.0.0.1', () => {
  ...
  log.info({
    toolsAvailable: startupValidation.tools.tools.filter(t => t.available).length,
    toolsTotal: startupValidation.tools.tools.length,
    apiKeysConfigured: startupValidation.apiKeys.configured.length,

...
const log = createModuleLogger('spiderFoot');
...
interface SpiderFootCommand {
    binary: string;
    baseArgs: string[];
}

async function resolveSpiderFootCommand(): Promise<SpiderFootCommand | null> {
    if (process.env.SPIDERFOOT_CMD) {
        // Handle custom command - split into binary and args if it's a python script
        const cmd = process.env.SPIDERFOOT_CMD;
        if (cmd.includes(' ')) {
            const parts = cmd.split(' ');
            return { binary: parts[0], baseArgs: parts.slice(1) };
        }
        return { binary: cmd, baseArgs: [] };
    }
    const candidates = [
    ...
    for (const cand of candidates) {
        try {
            if (cand.startsWith('/')) {
                ...
                // For Python scripts, return python3 as the binary with script as first arg
                if (cand.includes('.py')) {
                    return { binary: 'python3', baseArgs: [cand] };
                }
                return { binary: cand, baseArgs: [] };
            }
...
export async function runSpiderFoot(job: { domain: string; scanId: string }): Promise<number> {
    ...
    log.info(`[SpiderFoot] Starting scan for ${domain} (scanId=${scanId})`);

    const spiderFootCmd = await resolveSpiderFootCommand();
    if (!spiderFootCmd) {
        log.info('[SpiderFoot] [CRITICAL] Binary not found – module skipped');
    ...
    const { binary, baseArgs } = spiderFootCmd;
    ...
    if (missingKeys.length > 0) {
        ...
        log.info(`[SpiderFoot] [WARNING] ${warningText}`);
        await insertArtifact({
            type: 'scan_warning',
    ...
    log.info(`[SpiderFoot] API keys: HIBP ${mask(config.haveibeenpwnd_api_key)}, Chaos ${mask(config.chaos_api_key)} (Shodan/Censys handled by dedicated modules)`);
    ...
    // Build full argument array for execFileAsync (no shell interpolation)
    const args = [...baseArgs, '-q', '-s', sanitizedDomain, '-m', TARGET_MODULES, '-o', 'json'];
    log.info({ binary, args }, '[SpiderFoot] Executing with args:');
    ...
    try {
        ...
        // Use execFileAsync to avoid shell interpolation and prevent command injection
        const { stdout, stderr } = await execFileAsync(binary, args, { env, timeout: TIMEOUT_MS, maxBuffer: 20 * 1024 * 1024 });
        if (stderr) log.info({ stderr: stderr.slice(0, 400) }, '[SpiderFoot-stderr]');
        log.info(`[SpiderFoot] Raw output size: ${stdout.length} bytes`);
            ...
            switch (row.type) {
                ...
                case 'LINKED_URL_INTERNAL':
                    ...
                    if (url.includes('github.com') || url.includes('gitlab.com') || 
                        ...
                        log.info(`[SpiderFoot] Added to TruffleHog queue: ${row.data}`);
                    }
        ...
        if (linkUrls.length > 0) {
            log.info({ detail: linkUrls }, `[SpiderFoot] Collected linkUrls for TruffleHog`);
            await fs.writeFile(`/tmp/spiderfoot-links-${scanId}.json`, JSON.stringify(linkUrls, null, 2));
            log.info(`[SpiderFoot] Saved ${linkUrls.length} URLs to /tmp/spiderfoot-links-${scanId}.json for TruffleHog`);
        }
        ...
        log.info(`[SpiderFoot] ✔️ Completed – ${artifacts} artifacts`);
        return artifacts;
    } catch (err: any) {
        log.info({ err }, '[SpiderFoot] Scan failed');
        await insertArtifact({
            type: 'scan_error',

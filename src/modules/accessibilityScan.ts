...
// Enhanced logging
const log = createModuleLogger('accessibilityScan');
...
async function computePageHash(url: string): Promise<PageHashData | null> {
  try {
    ...
    log.info(`Failed to compute hash for ${url}: ${(error as Error).message}`);
    return null;
  }
}
...
async function hasAccessibilityChanged(domain: string, currentHashes: PageHashData[]): Promise<boolean> {
  try {
    ...
    log.info(`accessibility=change_detection domain="${domain}" status="starting_fresh"`);
    return true;
    ...
    for (const [url, currentHash] of currentHashMap) {
      ...
      if (!previousHash) {
        log.info(`accessibility=change_detected domain="${domain}" url="${url}" reason="new_page"`);
      ...
      if (currentHash.titleHash !== previousHash?.titleHash ||
        ...
        log.info(`accessibility=change_detected domain="${domain}" url="${url}" reason="content_changed"`);
        return true;
      }
    }
    ...
    for (const url of previousHashMap.keys()) {
      if (!currentHashMap.has(url)) {
        log.info(`accessibility=change_detected domain="${domain}" url="${url}" reason="page_removed"`);
        return true;
      }
    }
    
    log.info(`accessibility=no_change_detected domain="${domain}" pages=${currentHashes.length}`);
    return false;
    
  } catch (error) {
    log.info(`accessibility=change_detection_error domain="${domain}" error="${(error as Error).message}"`);
...
async function testPageAccessibility(url: string): Promise<AccessibilityPageResult> {
  ...
  if (process.env.ENABLE_PUPPETEER === '0') {
    log.info(`Accessibility test skipped for ${url}: Puppeteer disabled`);
    return { 
      url, 
      tested: false, 
      violations: [], 
  ...
  try {
    return await withPage(async (page) => {
      log.info(`Testing accessibility for: ${url}`);
      ...
      log.info(`Accessibility test complete for ${url}: ${violations.length} violations, ${results.passes.length} passes`);
    ...
    log.info(`Accessibility test error for ${url}: ${(error as Error).message}`);
    return { 
      url, 
      tested: false, 
      violations: [], 
...
export async function runAccessibilityScan(job: { domain: string; scanId: string }): Promise<number> {
  ...
  log.info(`Starting accessibility scan for domain="${domain}"`);
  
  // Handle Puppeteer disabled case
  if (process.env.ENABLE_PUPPETEER === '0') {
    log.info('Accessibility scan unavailable: Puppeteer disabled');
  ...
  try {
    ...
    log.info(`Discovered ${pagesToTest.length} pages to test for accessibility`);
    
    // STEP 1: Compute current page hashes for change detection
    log.info(`accessibility=hash_computation domain="${domain}" pages=${pagesToTest.length}`);
    ...
    if (!hasChanged) {
      ...
      log.info(`accessibility=skipped domain="${domain}" reason="no_changes_detected"`);
    ...
    log.info(`accessibility=running_full_scan domain="${domain}" reason="changes_detected"`);
    ...
    log.info(`Accessibility analysis complete: ${summary.totalViolations} violations (${summary.criticalViolations} critical, ${summary.seriousViolations} serious)`);
    ...
    log.info(`Accessibility scan completed: ${findingsCount} findings from ${summary.pagesSuccessful}/${summary.totalPages} pages in ${duration}ms`);
    
    return findingsCount;
    
  } catch (error) {
    const errorMsg = (error as Error).message;
    log.info(`Accessibility scan failed: ${errorMsg}`);

...
const log = createModuleLogger('aiPathFinder');
...
async function getTechStack(scanId: string, domain: string): Promise<TechStack> {
    ...
    const result = { rows: rows };
        ...
        log.debug({ techStack: defaultStack }, 'Detected tech stack');

    } catch (error) {
        log.warn({ err: error }, 'Error querying tech stack');
    }
...
async function generateIntelligentPaths(domain: string, techStack: TechStack): Promise<GeneratedPath[]> {
    if (!process.env.OPENAI_API_KEY) {
        log.info('No OpenAI API key - using fallback path generation');
        return generateFallbackPaths(techStack);
    }

    log.debug({ domain }, 'Preparing OpenAI request');
...
IMPORTANT: Return ONLY the JSON array, no additional text or explanation.`;
        ...
        log.debug({ durationMs: Date.now() - apiStart }, 'OpenAI response received');
        ...
        validPaths.forEach(path => {
            log.debug({ path: path.path, confidence: path.confidence }, 'AI suggested path');
        });

        log.info({ pathCount: validPaths.length }, 'Generated AI-powered paths');
        return validPaths.slice(0, MAX_PATHS_TO_GENERATE);

    } catch (error) {
        log.warn({ err: error }, 'Error generating AI paths, falling back to rule-based path generation');
        return generateFallbackPaths(techStack);
    }
}
...
function generateFallbackPaths(techStack: TechStack): GeneratedPath[] {
    ...
    log.debug({ pathCount: paths.length }, 'Generated fallback paths');
    return paths;
}
        ...
        const chunkResults = await Promise.allSettled(
            chunk.map(async (pathInfo) => {
                ...
                try {
                    ...
                    if (accessible) {
                        log.info({ url, status: response.status }, 'Found accessible path');
                    }
...
export async function runAiPathFinder(job: { domain: string; scanId?: string }): Promise<number> {
    ...
    log.info({ domain: job.domain, scanId: job.scanId }, 'Starting AI-powered path discovery');

    if (!job.scanId) {
        log.warn('No scanId provided - skipping AI path finding');
        return 0;
    }
    ...
    try {
        ...
        log.info({ accessibleCount: accessiblePaths.length, generatedCount: generatedPaths.length, durationMs: Date.now() - start }, 'AI path discovery completed');
        return accessiblePaths.length;

    } catch (error) {
        log.error({ err: error }, 'Error in AI path discovery');
        return 0;
    }
}
...
// Enhanced logging
const log = createModuleLogger('emailBruteforceSurface');
...
async function getEmailTargets(scanId: string, domain: string): Promise<string[]> {
  ...
  try {
    ...
    log.info(`Generated ${targets.size} email service targets`);
    return Array.from(targets).slice(0, MAX_TARGETS);
    
  } catch (error) {
    log.info(`Error getting email targets: ${(error as Error).message}`);
    return [];
  }
}
...
async function runNucleiEmailScan(targets: string[]): Promise<NucleiResult[]> {
  ...
  try {
    ...
    log.info(`Running Nuclei with ${EMAIL_TEMPLATES.length} email templates against ${targets.length} targets`);
    ...
    if (!result.success) {
      log.info(`Nuclei email scan failed with exit code ${result.exitCode}`);
      return [];
    }
    
    // Enhanced stderr logging - capture full output for better debugging
    if (result.stderr) {
      log.info(`Nuclei stderr: ${result.stderr}`);
    }
    
    log.info(`Nuclei email scan completed: ${result.results.length} findings`);
    return result.results as NucleiResult[];
    
  } catch (error) {
    log.info(`Nuclei email scan failed: ${(error as Error).message}`);
    return [];
  }
}
...
export async function runEmailBruteforceSurface(job: { domain: string; scanId: string }): Promise<number> {
  ...
  log.info(`Starting email bruteforce surface scan for domain="${domain}"`);
  ...
  try {
    ...
    if (targets.length === 0) {
      log.info('No targets found for email service scanning');
      return 0;
    }
    ...
    if (nucleiResults.length === 0) {
      log.info('No email services detected');
      return 0;
    }
    ...
    log.info(`Email service scan complete: ${nucleiResults.length} services found, ${summary.bruteforceTargets} bruteforce targets`);
    ...
    log.info(`Email bruteforce surface scan completed: ${findingsCount} findings in ${duration}ms`);
    
    return findingsCount;
    
  } catch (error) {
    const errorMsg = (error as Error).message;
    log.info(`Email bruteforce surface scan failed: ${errorMsg}`);

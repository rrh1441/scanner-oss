...
const log = createModuleLogger('documentExposure');
...
try {
  ...
  log.warn({ err }, 'pdf.worker.mjs not found');
}
...
async function gptRelevant(sample: string, sig: BrandSignature): Promise<boolean> {
  ...
  try {
    ...
    log.warn({ err }, 'GPT relevance error - fail-open');
    return true;
  }
}
...
async function gptIndustry(company: string, domain: string): Promise<IndustryGuard> {
  ...
  try {
    ...
    log.warn({ err }, 'GPT industry error - fail-open');
...
function score(content: string): { sensitivity: number; findings: string[] } {
  ...
  const cc = (content.match(/\b(?:\d[ -]*?){13,19}\b/g) ?? []) as string[];
...
async function downloadAndAnalyze(
  ...
  try {
    ...
    log.debug({ err, url: urlStr }, 'Document processing error');
    return null;
  } finally {
...
export async function runDocumentExposure(job: {
  ...
  log.info({ domain, companyName, scanId }, 'Starting document exposure scan');

  if (!process.env.SERPER_KEY) {
    log.warn('SERPER_KEY missing');
    return 0;
  }
  ...
  log.info({ maxQueries: MAX_SEARCH_QUERIES }, 'Cost control: limiting search queries');
  ...
  log.info({ domain, queryCount: allQueries.length }, 'Starting parallel Serper queries');
  ...
  const queryResults = await Promise.allSettled(
    allQueries.map(async ({query, category}, index) => {
      try {
        log.debug({ queryIndex: index + 1, query }, 'Serper API call');
        const response = await httpRequest({
          url: SERPER_URL,
          method: 'POST',
        ...
        log.debug({ queryIndex: index + 1, resultsCount: results.length }, 'Query returned results');
        return { category, query, results, success: true };
      } catch (error) {
        log.warn({ err: error, queryIndex: index + 1 }, 'Query failed');
        return { category, query, results: [], success: false, error };
      }
    })
  );
  
  log.info({ successfulQueries: queryResults.filter(r => r.status === 'fulfilled').length, durationMs: Date.now() - serperStart }, 'Serper queries completed');
  ...
  for (const result of queryResults) {
    ...
    for (const hit of results) {
      ...
      log.debug({ url: urlStr, platform }, 'Found document');
  ...
  log.info({ documentsFound: total, durationMs: Date.now() - start, queriesExecuted: allQueries.length, estimatedCostUsd: estimatedCost }, 'Scan completed');

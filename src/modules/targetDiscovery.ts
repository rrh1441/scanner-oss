...
const log = createModuleLogger('targetDiscovery');
...
export class TargetDiscovery {
  ...
  async buildTargets(scanId: string, domain: string): Promise<ClassifiedTarget[]> {
    ...
    const result = { rows: [] };      
      ...
      log.info(`buildTargets discovered=${discoveredCount} total=${targets.size} (html=${htmlCount}, nonHtml=${nonHtmlCount})`);
      
    } catch (error) {
      log.info(`buildTargets error: ${(error as Error).message}`);
    }
  ...
  async discoverThirdPartyOrigins(domain: string): Promise<ClassifiedTarget[]> {
    ...
    if (!puppeteerEnabled) {
      log.info(`thirdParty=skipped domain=${domain} reason="puppeteer_disabled"`);
      return [];
    }
    ...
    try {
      return await withPage(async (page) => {
        ...
        try {
          ...
          log.info(`thirdParty=navigation_fallback domain=${domain} error="${(navError as Error).message}"`);
          await page.goto(`https://${domain}`, { 
            timeout: pageTimeout,
        ...
        log.info(`thirdParty=discovered domain=${domain} total=${limitedOrigins.length} (html=${htmlCount}, nonHtml=${nonHtmlCount})`);
        
        return classifiedTargets;
      });
      
    } catch (error) {
      log.info(`thirdParty=error domain=${domain} error="${(error as Error).message}"`);
      return [];
    }
  }

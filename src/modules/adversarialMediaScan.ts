...
// Enhanced logging
const log = createModuleLogger('adversarialMediaScan');
...
async function executeSearchQuery(query: string, apiKey: string): Promise<SerperSearchResult[]> {
  try {
    log.info(`Executing search query: "${query.substring(0, 50)}..."`);
    ...
    log.info(`Search query failed: ${errorMsg}`);
...
function processSearchResults(
  ...
  log.info(`Filtered to ${recentArticles.length} recent articles (within ${WINDOW_DAYS} days)`);
...
export async function runAdversarialMediaScan(job: { 
  ...
  log.info(`Starting adversarial media scan for company="${company}" domain="${domain}"`);
  
  // Validate inputs
  if (!company || !domain) {
    log.info('Missing required parameters: company and domain');
    return 0;
  }
  ...
  if (!apiKey) {
    log.info('SERPER_KEY not configured, emitting error and exiting');
  ...
  try {
    ...
    log.info(`Generated ${searchQueries.length} search queries`);
    ...
    log.info(`Collected ${uniqueResults.length} unique articles (${allResults.length - uniqueResults.length} duplicates removed)`);
    ...
    log.info(`Categorized ${totalArticles} articles into ${categoryCount} risk categories`);
    ...
    log.info(`Adversarial media scan complete: ${findingsCount} findings generated in ${duration}ms`);
    
    return findingsCount;
    
  } catch (error) {
    const errorMsg = (error as Error).message;
    log.info(`Adversarial media scan failed: ${errorMsg}`);

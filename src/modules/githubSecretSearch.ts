  ...
  try {
    const { body, statusCode, headers } = await request(url, {
      ...
      headers: {
        ...
        'User-Agent': 'SecurityScanner-Scanner/1.0 (+https://simplcyber.com)',
...
/**
 * Run GitHub secret search as a scan module
 * Searches for exposed secrets related to the target domain
 */
export async function runGithubSecretSearch(job: {
  domain: string;
  scanId: string;
}): Promise<number> {
  ...
  if (!isConfigured()) {
    log.info({ scanId }, 'GitHub secret search skipped - GITHUB_TOKEN not configured');
    return 0;
  }

  log.info({ domain, scanId }, 'Starting GitHub secret search');

  let findingsCount = 0;

  try {
    // Search for high-value patterns related to this domain
    const results = await searchHighValuePatterns((pattern, progress) => {
      log.debug({ pattern, progress, scanId }, 'Search progress');
    });

    // Process results and create findings
    for (const [patternName, leads] of results) {
      if (leads.length === 0) continue;

      for (const lead of leads) {
        // Determine severity based on pattern
        let severity: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' = 'HIGH';
        if (patternName.includes('AWS') || patternName.includes('database') || patternName.includes('Private Key')) {
          severity = 'CRITICAL';
        } else if (patternName.includes('webhook') || patternName.includes('Publishable')) {
          severity = 'MEDIUM';
        }

        const artifactId = await insertArtifact({
          type: 'github_secret_lead',
          val_text: `${patternName} found in ${lead.repo}`,
          severity,
          src_url: lead.url,
          meta: {
            scan_id: scanId,
            scan_module: 'github_secret_search',
            domain,
            pattern_name: patternName,
            repository: lead.repo,
            file_path: lead.path,
            file_name: lead.filename,
            secret_preview: lead.preview,
        ...
        await insertFinding({
          ...
          recommendation: `Review and rotate the exposed ${patternName}. Remove the secret from the repository and use environment variables or a secrets manager instead.`,
          description: `Potential ${patternName} found in GitHub repository ${lead.repo} at path ${lead.path}`,
          scan_id: scanId,
          severity,
          type: 'EXPOSED_SECRET',
        });
    ...
    log.info({ domain, scanId, findingsCount }, 'GitHub secret search completed');

  } catch (error) {
    log.error({ err: error, domain, scanId }, 'GitHub secret search failed');

    await insertArtifact({
      type: 'scan_error',
      val_text: `GitHub secret search failed: ${(error as Error).message}`,
      severity: 'MEDIUM',
      meta: { scan_id: scanId, module: 'github_secret_search', domain },
    });
  }

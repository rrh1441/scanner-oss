...
const log = createModuleLogger('scanGitRepos');
...
export async function scanGitRepo(
    ...
    log.info(`[trufflehog] [Git Scan] Starting scan for repository: ${url} (depth: ${depth})`);
    ...
    try {
        ...
        if (stderr) {
            log.info({ detail: stderr }, `[trufflehog] [Git Scan] [STDERR] for ${url}`);
        }
        
        const findings = await processTrufflehogOutput(stdout, 'git', url, scanId);
        log.info(`[trufflehog] [Git Scan] Completed scan for ${url}: ${findings} findings`);
        
        return findings;
    } catch (err) {
        log.info({ err: err as Error }, `[trufflehog] [Git Scan] Error scanning repository ${url}`);
        return 0;
    }
}
...
export async function scanGitRepos(
    ...
    log.info(`[trufflehog] [Git Scan] Starting scan of ${reposToScan.length} repositories (max: ${maxRepos})`);
    ...
    for (const url of reposToScan) {
        try {
            ...
            log.info({ err: error as Error }, `[trufflehog] [Git Scan] Failed to scan repository ${url}`);
        }
    }
    
    log.info(`[trufflehog] [Git Scan] Completed scan of ${reposToScan.length} repositories: ${totalFindings} total findings`);

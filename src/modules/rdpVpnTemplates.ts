...
// Enhanced logging
const log = createModuleLogger('rdpVpnTemplates');
...
async function getTargetUrls(scanId: string, domain: string): Promise<string[]> {
  ...
  try {
    ...
    log.info(`Generated ${targets.size} target URLs for RDP/VPN scanning`);
    return Array.from(targets).slice(0, MAX_TARGETS);
    
  } catch (error) {
    log.info(`Error getting target URLs: ${(error as Error).message}`);
    return [];
  }
}
...
async function runNucleiRdpVpn(targets: string[]): Promise<NucleiResult[]> {
  ...
  try {
    ...
    log.info(`Running Nuclei with ${RDP_VPN_TEMPLATES.length} RDP/VPN templates against ${targets.length} targets`);
    ...
    if (!result.success) {
      log.info(`Nuclei RDP/VPN scan failed with exit code ${result.exitCode}`);
      return [];
    }
    ...
    log.info(`Nuclei RDP/VPN scan completed: ${result.results.length} findings`);
    return result.results as NucleiResult[];
    
  } catch (error) {
    log.info(`Nuclei RDP/VPN scan failed: ${(error as Error).message}`);
    return [];
  }
}
...
export async function runRdpVpnTemplates(job: { domain: string; scanId: string }): Promise<number> {
  ...
  log.info(`Starting RDP/VPN templates scan for domain="${domain}"`);
  ...
  try {
    ...
    if (targets.length === 0) {
      log.info('No targets found for RDP/VPN scanning');
      return 0;
    }
    ...
    if (nucleiResults.length === 0) {
      log.info('No RDP/VPN vulnerabilities detected');
      return 0;
    }
    ...
    log.info(`RDP/VPN templates scan completed: ${findingsCount} findings in ${duration}ms`);
    
    return findingsCount;
    
  } catch (error) {
    const errorMsg = (error as Error).message;
    log.info(`RDP/VPN templates scan failed: ${errorMsg}`);

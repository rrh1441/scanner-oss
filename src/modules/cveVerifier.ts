...
const log = createModuleLogger('cveVerifier');
...
async function getUbuntuFixedVersion(cve: string): Promise<string | undefined> {
  ...
  try {
    log.info(`Checking Ubuntu fix data for ${cve}`);
    ...
    if (fixedVersion) {
      log.info(`Ubuntu fix found for ${cve}: ${fixedVersion}`);
    } else {
      log.info(`No Ubuntu fix data found for ${cve}`);
    }
    
    return fixedVersion;
  } catch (error) {
    log.info(`Error fetching Ubuntu fix data for ${cve}: ${(error as Error).message}`);
...
async function nucleiSupports(cve: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (templatePath) {
      log.info(`Found Nuclei template for ${cve}: ${templatePath}`);
    } else {
      log.info(`No Nuclei template found for ${cve}`);
    }
    
    return templatePath;
  } catch (error) {
    log.info(`Error searching for Nuclei template ${cve}: ${(error as Error).message}`);
...
async function runNuclei(
  ...
  try {
    log.info(`Running Nuclei template ${template} against ${host}`);
    ...
    if (hasMatch) {
      log.info(`Nuclei confirmed vulnerability: ${template} matched ${host}`);
    } else {
      log.info(`Nuclei found no vulnerability: ${template} did not match ${host}`);
    }
    
    return hasMatch;
  } catch (error) {
    log.info(`Nuclei execution failed for ${template}: ${(error as Error).message}`);
    return false;
  }
}
...
export function getCommonCVEsForService(service: string, version: string): string[] {
  ...
  if (!cveList) {
    log.info(`No CVE database found for service: ${service}`);
    return [];
  }
  ...
  if (!cleanVersion) {
    log.info(`Could not parse version: ${version}, returning all CVEs for ${service}`);
  ...
  for (const cve of cveList) {
    ...
    if (releaseYear && releaseYear > cve.publishedYear + 1) { // +1 year buffer
      log.info(`CVE ${cve.id} excluded: software version ${version} (${releaseYear}) released after CVE (${cve.publishedYear})`);
      continue;
    }
    ...
    try {
      ...
      if (semver.satisfies(cleanVersion, cve.affectedVersions)) {
        applicableCVEs.push(cve.id);
        log.info(`CVE ${cve.id} applicable to ${service} ${version}`);
      } else {
        log.info(`CVE ${cve.id} not applicable: version ${version} outside range ${cve.affectedVersions}`);
      }
    } catch (error) {
      log.info(`Error checking version range for ${cve.id}: ${(error as Error).message}`);
      // Include on error for safety, but log the issue
      applicableCVEs.push(cve.id);
    }
  }
  
  log.info(`Service ${service} v${version}: ${applicableCVEs.length}/${cveList.length} CVEs applicable`);
...
export function extractCVEsFromNucleiOutput(nucleiJson: string): string[] {
  ...
  try {
    ...
    log.info(`Error parsing Nuclei output for CVE extraction: ${(error as Error).message}`);
  }

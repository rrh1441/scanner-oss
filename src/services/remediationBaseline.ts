...
const BASELINE_GUIDANCE: Record<string, BaselineGenerator> = {
  tlsScan: ({ priority }) => ({
    ...
    verification: [
      'Re-run SecurityScanner TLS scan and confirm no HIGH/MEDIUM findings',
  ...
  configExposureScanner: ({ priority }) => ({
    ...
    verification: [
      ...
      'Re-run SecurityScanner config exposure module to ensure zero leaks.'
    ],
    steps: normaliseSteps([
  ...
  nextjsRscScan: ({ priority, finding }) => ({
    priority,
    timeline: priority === 'Immediate' ? 'Patch and redeploy within 24-48 hours' : 'Update framework versions this sprint',
    description: 'Patch Next.js / React Server Components to address critical RCE vulnerabilities.',
    businessImpact: 'Remote code execution in your primary web application can lead to full compromise, data breach, and infrastructure pivoting.',
    ownerHint: 'Application development / platform engineering',
    effort: 'High',
    verification: [
      'Verify all Next.js and React Server Components packages are on patched versions (Next.js 15.0.5+/16.0.7+, React 19.0.1+).',
      'Confirm applications were rebuilt and redeployed after the patch.',
      'Re-run external scanner to confirm domain no longer flags as potentially exposed.',
      'Verify WAF rules for React/Next.js RCE patterns are enabled in blocking mode.'
    ],
    steps: normaliseSteps([
      'Inventory all internet-facing services running Next.js App Router or React Server Components.',
      'Update to patched framework versions: Next.js 15.0.5+ or 16.0.7+, React Server Components 19.0.1+.',
      'Rebuild and redeploy all affected applications including production, staging, and demo environments.',
      'Enable WAF rules or IPS signatures targeting React Server Components and Next.js RCE exploit patterns.',
      'Review application logs for unusual traffic patterns during the unpatched window.',
      'If suspicious activity found, rotate all application secrets (database credentials, API keys, OAuth tokens).'
    ]),
    additionalHardening: [
      'Implement least-privilege for application runtime roles.',
      'Segment critical data stores from public-facing apps.',
      'Restrict outbound egress from web application containers.'
    ],
    references: [
      { label: 'Next.js Security Advisories', url: 'https://github.com/vercel/next.js/security/advisories' },
      { label: 'React Security Documentation', url: 'https://react.dev/reference/react/security' }
    ],
    source: 'module',
    generatedAt: new Date().toISOString(),
    metadata: {

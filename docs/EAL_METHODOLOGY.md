# SecurityScanner EAL (Expected Annual Loss) Methodology
...
SecurityScanner calculates **Expected Annual Loss (EAL)** using a FAIR-inspired risk model anchored to **$300,000** — the typical cost of a serious cybersecurity incident for a small-to-medium business based on 2023-2025 breach cost data.
...
### How SecurityScanner EAL Compares

| Model | Anchor Point | Strengths | Limitations |
|-------|--------------|-----------|-------------|
| **SecurityScanner EAL** | $300K SMB breach | FAIR-inspired, prevalence-weighted, family-capped | Conservative (may underestimate for enterprises) |
...
**SecurityScanner is optimized for SMBs** ($1M-$50M revenue) and intentionally conservative to avoid "scare tactics" while still representing real financial risk.
...
**SecurityScanner EAL works the same way**:
...
- SecurityScanner now **verifies active employment** for infostealer findings, increasing confidence in exposure validity
